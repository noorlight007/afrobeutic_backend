...
class SalonBookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["booking_duration", "booking_id", "cancelled_by", "source"]
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            customer_source = customer.get("source", "Admin")
            ...
            validated_data["source"] = BookingSource.MANUAL
...
class SalonChairBookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["booking_duration", "booking_id", "source"]
...
class SalonLookBookSerializer(serializers.ModelSerializer):
    ...
    def validate_images(self, value):
        ...
        """
        images = SalonMedia.objects.filter(booking=self.instance)
        if images.count() + len(value) > 3:
            raise serializers.ValidationError(
                "A maximum of three images can be uploaded for a booking."
            )
