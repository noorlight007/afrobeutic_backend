...
from apps.support.choices import EnquirySource
...
from apps.salon.choices import BookingStatus, ChairStatus, CustomerType, BookingSource
...
def get_services_and_products(salon: Salon, gender_filter: str = None) -> dict:
    services_qs = salon.salon_services.select_related("category")
    if gender_filter:
    ...
    products = []
    for p in salon.salon_products.select_related("category"):
        products.append(
            {
                "id": str(p.uid),
                "name": p.name,
